...
public interface ResourceMapper extends BaseMapper<Resource> {
    @Insert("""
            INSERT INTO base_org_resource
              (id, code, type, name, url, method, description, application, product_code, source, status, handler,
               first_seen_at, last_seen_at, missing_since, app_version,
               created_time, updated_time, created_by, updated_by)
            VALUES
              (#{id}, #{code}, #{type}, #{name}, #{url}, #{method}, #{description}, #{application}, #{productCode},
            ...
            ON DUPLICATE KEY UPDATE
              ...
              description = VALUES(description), application = VALUES(application), product_code = VALUES(product_code), source = VALUES(source),
