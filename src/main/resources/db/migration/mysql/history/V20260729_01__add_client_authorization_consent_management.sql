...
-- 客户端授权记录复用 Spring Authorization Server 的 consent 表。
INSERT INTO base_org_resource
    (id, name, code, type, url, method, description,
     created_time, updated_time, created_by, updated_by)
VALUES
    (334, '查询客户端授权记录', 'auth:consent:query', 'authorization',
     '/api/auth/authorization-consents/conditions', 'POST',
     '分页查询用户授予OAuth2客户端的权限记录',
     now(), now(), 'system', 'system'),
    (335, '查看客户端授权记录', 'auth:consent:view', 'authorization',
     '/api/auth/authorization-consents', 'GET',
     '查看用户授予OAuth2客户端的权限详情',
     now(), now(), 'system', 'system'),
    (336, '删除客户端授权记录', 'auth:consent:remove', 'authorization',
     '/api/auth/authorization-consents', 'DELETE',
     '删除客户端授权同意，用户下次授权时需要重新同意',
     now(), now(), 'system', 'system')
...
JOIN base_org_resource resource ON resource.id IN (334, 335, 336)
...
INSERT INTO base_org_menu
    (id, parent_id, type, href, icon, name, description, order_num,
     created_time, updated_time, created_by, updated_by)
VALUES
    (233, 110, 'BUTTON', '', '', '删除客户端授权记录',
     '{"perm":"auth:consent:remove"}',
...
SELECT 100000 + role.id * 1000 + menu.id, role.id, menu.id,
       now(), now(), 'system', 'system'
FROM base_org_role role
JOIN base_org_menu menu ON menu.id = 233
WHERE role.id IN (101, 103)
ON DUPLICATE KEY UPDATE
    updated_time = now(),
    updated_by = 'system';
